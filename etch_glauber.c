 ...
 /*   
  for(l = 0; l < npass; l++){
    if(l%config_freq==0) printf("Pass number %ld\n", l+1);
      int tmp;
      tmp = mc_move_glauber(lat, surf, vac);
    if(tmp == 1){
      mc_insert++;
      mc_accept++;
    }
...
int mc_move_glauber(nn_vec ***r, triple *surf, triple *vac){
  ...
  if(xsi > 0.5){ //try insertion
    ...
    double f = exp(mu*beta + epsilon*dM*beta)*alpha;
    double weight = 1.0/(1.0 + 1.0/f); //glauber weight: 1/(1+1/f)

    //Randomly generate number and compare to glauber weight
  ...
  /*******************************************DELETION STEP******************************************/
    ...
    double g = exp(-mu*beta - epsilon*dM*beta)*alpha;
    double weight = 1.0/(1.0 + 1.0/g); //glauber weight: 1/(1+1/g)
