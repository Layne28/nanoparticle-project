...
int main(int argc, char *argv[]){
  ...
  for(i = 0; i < 2*nl; i++) {
    for(j = 0; j < 2*nl; j++){
      for(k = 0; k < 2*nl; k++){
	fscanf(in, "%s\t" "%lf\t" "%lf\t" "%lf\t" "%d\t"  "%d\n", atom_curr, &temp1, &temp2, &temp3, &lat[i][j][k].n, &lat[i][j][k].occ);
	if(lat[i][j][k].occ==1){
	  Ntot++;
  ...
  for(l = 0; l < npass; l++){
       if(l%10==0) printf("Pass number %d\n", l+1);
       int tmp;
...
int mc_move(nn_vec ***r, triple *surf, triple *vac){
  ...
  if(xsi > 0.5){ //try insertion
    ...
    nbs_vec *nbs_temp = (nbs_vec *) calloc((size_t)NN, sizeof(nbs_vec)); //for debugging
...
int mc_move_test(nn_vec ***r, triple *surf, triple *vac){
  ...
  if(xsi > 0.5){ //try insertion
    ...
    nbs_vec *nbs_temp = (nbs_vec *) calloc((size_t)12, sizeof(nbs_vec)); //for debugging
