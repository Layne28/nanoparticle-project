...
int main(void){
  ...
  for(l = 0; l < (npass/config_freq+2); l++){
    traj[l] = (nn_vec ***) calloc((size_t)nl, sizeof(nn_vec**));
    for(i = 0; i < nl; i++){
      traj[l][i] = (nn_vec **) calloc((size_t)nl, sizeof(nn_vec*));
      for(j = 0; j < nl; j++){
	traj[l][i][j] = (nn_vec *) calloc((size_t)nl, sizeof(nn_vec));
      }
    }
  }
...
int mc_move(nn_vec ***r, triple *surf, triple *vac){
  ...
  /*******************************************DELETION STEP******************************************/
    ...
    nbs_vec *nbs = (nbs_vec *) calloc((size_t)NN, sizeof(nbs_vec));
...
void neighbors(nn_vec ***r, nbs_vec *nbs, int i, int j, int k){
  if(i==0 || j==0 || k==0 || i==(nl-1) || j==(nl-1) || k==(nl-1)){
    ...
    nbs[0].z = k;
    nbs[0].n = r[i-1][j][k].n;
    nbs[0].occ = r[i-1][j][k].occ;
 
    nbs[1].x = i+1;
    nbs[1].y = j;
    nbs[1].z = k;
    nbs[1].n = r[i+1][j][k].n;
    nbs[1].occ = r[i+1][j][k].occ;
 
    nbs[2].x = i;
    nbs[2].y = j-1;
    nbs[2].z = k;
    nbs[2].n = r[i][j-1][k].n;
    nbs[2].occ = r[i][j-1][k].occ;
  
    nbs[3].x = i;
    nbs[3].y = j+1;
    nbs[3].z = k;
    nbs[3].n = r[i][j+1][k].n;
    nbs[3].occ = r[i][j+1][k].occ;
 
    nbs[4].x = i;
    nbs[4].y = j;
    nbs[4].z = k-1;
    nbs[4].n = r[i][j][k-1].n;
    nbs[4].occ = r[i][j][k-1].occ;

    nbs[5].x = i;
    nbs[5].y = j;
    nbs[5].z = k+1;
    nbs[5].n = r[i][j][k+1].n;
    nbs[5].occ = r[i][j][k+1].occ;
}
...
void print_final(nn_vec ***r) {
  ...
  for(i = 0; i < nl; i++) {
    for(j = 0; j < nl; j++){
      for(k = 0; k < nl; k++){
	      if(r[i][j][k].occ == 0) fprintf(f, "H\t%d\t%d\t%d\t%d\t%d\n", (i-nl/2), (j-nl/2), (k-nl/2), r[i][j][k].n, r[i][j][k].occ);
